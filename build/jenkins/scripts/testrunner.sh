...
#		Debug - (Default) Will build Default and DefaultARM,  run tests for Default. 
#		Release - (Release) Will build Release and ReleaseARM,  run tests for Release.
...
runTestsForProject() {
    ...
    if [ -e "${PROJECT_ROOT}/Test${projectName}" ]; then
        testProjectName="Test${projectName}"
    elif [ -e "${PROJECT_ROOT}/${projectName}Test" ]; then
...
if [ "$3" != "test" ]; then
	...
	find ${PROJECT_ROOT} -name ${CONFIG}ARM |grep -v sharedlibs |xargs rm -rf
fi
...
cd ${PROJECT_ROOT}; make -f maketest.mk PLT=${CONFIG}ARM all
if [ $? -ne 0 ]; then
	echo "******************************************************************************"
	echo "*************** ${CONFIG}ARM build FAILED ****************************************"
	echo "******************************************************************************"
	exit 1
fi
